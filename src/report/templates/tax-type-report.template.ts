...
export function generateTaxTypeHtmlTemplate(
  rows: TaxTypeRow[],
  ...
  const totalApplications = rows.reduce((s, r) => s + r.applicationCount, 0);
...
<body>
  <div class="header">
    ...
    <h2>Tax Type Analysis Report</h2>
  ...
  <table>
    ...
    <tbody>
      ${tableRows || '<tr><td colspan="8" style="text-align:center; padding: 20px;">No data found.</td></tr>'}
      <tr class="total-row">
        <td></td>
        <td>TOTAL</td>
        <td class="num">${totalAppeals}</td>
        <td class="num">${totalApplications}</td>
        <td class="num">${totalAll}</td>
        <td class="num">${rows.reduce((s, r) => s + r.pendingCount, 0)}</td>
        <td class="num">${rows.reduce((s, r) => s + r.decidedCount, 0)}</td>
        <td></td>
      </tr>
    </tbody>
  </table>
